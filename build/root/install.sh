...
# run nicotine
/usr/local/bin/portget.sh --application-name 'nicotineplus' --application-port "${APPLICATION_PORT}" /usr/bin/nicotine
EOF
...
cat <<'EOF' > /tmp/menu_heredoc
    <item label="Nicotine+">
    <action name="Execute">
      <command>/usr/local/bin/portget.sh --application-name 'nicotineplus' --application-port "${APPLICATION_PORT}" /usr/bin/nicotine</command>
      <startupnotify>
        <enabled>yes</enabled>
      </startupnotify>
    </action>
